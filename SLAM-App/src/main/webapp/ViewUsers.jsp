...
<body class="w3-theme-d1">
	...
	<script>
		...
		function submitUnFollow() {
			document.getElementById("unfollowUser").submit();
		}
	</script>
	<!-- Page Container -->
	<div class="w3-container w3-content w3-round-xlarge"
		...
		<div class="w3-left-align w3-large">
			<c:if test = "${!isAnonymous}">
				<c:if test="${isAdmin}">
				  ...
				  <p><a class="w3-button w3-hide-small w3-padding-large w3-hover-white" title="Search User"><i class="fa fa-search w3-margin-right"></i>Search User</a><p>
			...
			<div class="w3-col m6 w3-theme-d1 ">
				...
				<div
					...
					<a href="MenuController"><img src="imgs/logoIcon.png"
					...
					<div class="w3-col m12">
						<div class="w3-card w3-round w3-white w3-theme-l4">
							<div class="w3-container ">
								<form id="form" action="GetUsers" method="POST">
									<div class="w3-display-container">
										<input id="palabra" name="palabra" placeholder="Search User"
											class="w3-border w3-padding w3-transparent"
											style="width: 80%;"></input>
										<button type="submit" class="w3-button w3-display-right"
				...
				<c:forEach var="u" items="${users}">
					<div id="${u.user}"
						class="w3-container w3-card w3-round w3-white w3-center w3-section">
						<p>Friend Suggestion</p>
						<img src="imgs/avatar6.png" alt="Avatar" style="width: 10%"><br>
						<div>
							<a href="GetUserInfo?usr=${u.user}">${u.user}</a><br> <i>${u.firstName}
								${u.lastName}</i>
							<c:set var="isFollowing" value="false" />
							<c:forEach var="f" items="${followingUsers}">
								<c:if test="${u.user eq f.user}">
							...
							<c:if test="${not isFollowing}">
								<c:if test="${!isAnonymous}">
									<c:if test="${!isAdmin}">
										<form id="followUser" action="FollowUser?user=${f.user}"
											method="POST">
											<button type="submit" Onclick="submitFollow()"
												class="w3-button">
							...
							<c:if test="${isFollowing}">
								<c:if test="${!isAnonymous}">
									<c:if test="${!isAdmin}">
										<form id="unfollowUser" action="UnFollowUser?user=${f.user}"
											method="POST">
											<button type="submit" Onclick="submitUnFollow()"
												class="w3-button">
							...
							<c:if test="${isAdmin}">
							  <form action="DelUser?user=${u.user}" method="POST">
